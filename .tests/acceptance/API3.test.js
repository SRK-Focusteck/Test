...
describe('Acceptance Tests for API1 endpoint', () => {
    const API_BASE_URL = process.env.API_BASE_URL + 'api3';
    ...
    it('should return correct response body for API3', async function () {
        this.timeout(30000);

        const apiResponse = await chai.request(API_BASE_URL).get('');
        expect(apiResponse).to.have.status(200);
        expect(apiResponse.body).to.be.an('object');
        expect(apiResponse.body).to.have.property('message', 'Response from API 3');
        expect(apiResponse.body).to.have.property('apiCode', 'API3');
    });
});
